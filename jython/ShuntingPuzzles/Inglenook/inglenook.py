...
class Inglenook:
    ...
    def moveTruckToDesiredPosition(self, truckNo, destSiding):
        ...
        if self.truckInSiding(truckNo, destSiding):
            ...
            if not self.truckAtPositionFromEnd(truckNo, destSiding, desiredPosition + 1):
                ...
                if (self.truckAtPositionFromEnd(truckNo, destSiding, refPos) or
                    ...
                    msg = "move " + str(noTrucks) + " from " + str(destSiding) + " to " + str(freeBranch) + " as truck is in position " + str(mypos)
                    yield ["display_message", msg]

                    for p in self.moveTrucksCreatingYieldStatements(noTrucks, destSiding, freeBranch): yield p
                    msg = "b1 refpos " +str(refPos+2)
                    yield ["display_message", msg]
        ...
        if (not self.truckInSiding(truckNo, destSiding)):
            msg =  "truck " + str(truckNo) + " not in siding (destSiding) " + str(destSiding)
            yield ["display_message", msg]
        ...
        msg = "end of moveTruckToDesiredPosition " + str(truckNo) + " " + str(destSiding)
        yield ["display_message", msg]
    ...
    def insertTruck(self, from_branch, destination_branch, refPos) :
        ...
        msg = "insertTruck: " + "from_branch " + str(from_branch) + " destination_branch " + str(destination_branch) + " refPos " + str(refPos)
        yield ["display_message", msg]
        ...
        if no_trucks_to_move > 2:
            ...
            if self.noSpacesinBranch(other_branch == 0):
                ...
                if self.noSpacesinBranch(other_branch < noTrucks1):
                    self.moveTrucksOneByOneCreatingYieldStatements(self.noOfTrucksInBranch(from_branch), from_branch, other_branch)
                    # move trucks on from branch to other branch
                    temp = from_branch
                    from_branch = other_branch
                    other_branch = temp
                for p in self.moveTrucksOneByOneCreatingYieldStatements(noTrucks1, destination_branch, other_branch): yield p

        for p in self.moveTrucksOneByOneCreatingYieldStatements(min(no_trucks_to_move, 2), destination_branch, 4): yield p

        # move truck from fromBranch to head
        no_trucks_to_move = 1
        for p in self.moveTrucksOneByOneCreatingYieldStatements(no_trucks_to_move, from_branch, 4): yield p
        ...
        msg = "move " +str(no_trucks_to_move1) + " trucks from head to toBranch"
        yield ["display_message", msg]
        for p in self.moveTrucksOneByOneCreatingYieldStatements(no_trucks_to_move1, 4, destination_branch): yield p
        ...
        if no_trucks_to_move2 > 0:
            msg = "move2: move " +str(no_trucks_to_move2) + " trucks from head to toBranch"
            yield ["display_message", msg]
        ...
        if (no_trucks_to_move2 != 0):
            ...
            for p in self.moveTrucksOneByOneCreatingYieldStatements(no_trucks_to_move2, 4, freeBranch): yield p

    def moveTrucksCreatingYieldStatements(self, noTrucks, fromBranch, destBranch):

        msg = "moveTrucks noTrucks: " + str(noTrucks) + " fromBranch " + str(fromBranch) + " destBranch " + str(destBranch)
        yield ["display_message", msg]
        #print(self.pegs)
        ...
        msg = "finished moveTrucksCreatingYieldStatements"
        yield ["display_message", msg]
