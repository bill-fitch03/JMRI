...
# #@print_name()
# def uncouple2(self, branch, operation):     #uncouple seems to be a reserved word, so use uncouple1
#
#     # we have just gone to the dropoff position
#     # if we were going from 4 to branch (DROPOFF), then we need to go back a bit from branch to 4 where the magnet will uncouple we do this by counting 1 truck
#     # if we were going from branch to 4 (PICKUP) we will have got to the dropoff position
#     #logging = "print"
#
#     self.indent()
#     self.myprint("in uncouple")
#     self.myprint("operation = " , operation)
#     sensor = self.setSensor(branch)
#     self.setSpeed(self.stop)
#
#     if operation == "PICKUP":
#         self.myprint("in pickup")
#         #go back to uncouple position by counting active
#         direction = self.setDirection(branch, 4)
#         noTrucksToCount = 1  # starts from 0 ( we are in a truck so that is 0
#         self.countTrucksActive(noTrucksToCount, sensor, direction)
#         self.setSpeed(stop)
#     else:
#         # in PICKUP
#
...
#     self.set_delay_if_not_simulation(1000)
#     #now go to dropoff
#     self.setPointsAndDirection(4, branch)
#     self.setSpeedSetDelay(self.uncouple,50)
#
#     # #whiz forward
#     # self.myprint("uncouple forward 50")
...
# #        self.countTrucksInactive(noTrucksToCount, sensor)
    ...
    # self.myprint("countTrucksActive returned")
    # self.setSpeed(self.stop)
    # self.set_delay_if_not_simulation(100)
    # self.mysound()
    # self.set_delay_if_not_simulation(100)
    # self.mysound()
    # self.set_delay_if_not_simulation(100)
    ...
    # # if operation == "DROPOFF":
    # # #whiz forward if Dropoff
        # # self.myprint("fast forward DROPOFF 100")
